...
/**
 * Generate points for a specific segment
 * @param segment The segment to generate points for
 * @param pointsPerSegment Number of points to generate (higher = smoother)
 * @returns Array of x,y coordinate points
 */
function generateSegmentPoints(segment: FittedSegment, pointsPerSegment: number = 10): Array<{x: number, y: number}> {
  const points: Array<{x: number, y: number}> = [];

  for (let i = 0; i <= pointsPerSegment; i++) {
    const t = segment.start_t + (i / pointsPerSegment) * (segment.end_t - segment.start_t);
    const y = segment.coeffs.slope * t + segment.coeffs.intercept;
    points.push({ x: t, y });
  }
...
/**
 * Generate a QuickChart.io URL to visualize segments as separate series
 * @param segments Array of fitted line segments
 * @returns URL for chart visualization
 */
function generateChartUrl(segments: FittedSegment[]): string {
  ...
  const datasets = segments.map((segment, index) => {
    ...
    return {
      label: `Segment ${index + 1} (t=${segment.start_t} to t=${segment.end_t})`,
      data: points,
      showLine: true,
      fill: false,
      ...
      pointRadius: 3,
      tension: 0 // Important: use 0 for linear segments (no curve)
    };
  });
  ...
  const config = {
    ...
    options: {
      ...
      plugins: {
        title: {
          display: true,
          text: 'Segmented Piecewise Linear Function'
        },
        tooltip: {
          callbacks: {
            label: (context: any) => {
              const datasetLabel = context.dataset.label || '';
              return `${datasetLabel}: (${context.parsed.x.toFixed(1)}, ${context.parsed.y.toFixed(1)})`;
            }
          }
        },
        legend: {
...
/**
 * Fetch the chart as a PNG and save it locally (Node.js only)
 * @param segments Array of fitted line segments
 * @param filename The filename to save the image as
 * @returns Promise that resolves when the image is saved
 */
export async function fetchAndSaveChart(segments: FittedSegment[], filename: string = 'segment-chart.png'): Promise<void> {
  const { promises: fsPromises } = fs;

  try {
    const chartUrl = generateChartUrl(segments);
